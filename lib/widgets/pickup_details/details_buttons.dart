  ...
  Widget build(BuildContext context) {
    ...
    Widget weightButton() {
      return RaisedButton.icon(
        onPressed: pickup.status != Status.STARTED && pickup.status != Status.REJECTED ? null : onWeightPress,
        icon: Icon(Icons.edit),
        label: Text(localizations.translate("pickupDetailsButtonRegisterWeight")),
      );
    }
    ...
    Widget showNhDocButton() {
      return RaisedButton.icon(
        ...
        label: Text(localizations.translate("pickupDetailsButtonShowNHDocument")),
      );
    }
    ...
    Widget generateNhDocButton() {
      return RaisedButton.icon(
        ...
        label: Text(localizations.translate("pickupDetailsButtonGenerateNHDocument")),
      );
    }

    Widget scanNhDocButton() {
      return RaisedButton.icon(
        onPressed: (pickup.status == Status.STARTED || pickup.status == Status.REJECTED)
            ? () async {
        ...
        label: Text(localizations.translate("pickupDetailsButtonRegisterNHDocument")),
      );
    }

    List<Widget> nhdocButtons() {
      if (pickup.externalNHDocFormat == null || pickup.externalNHDocFormat == FileFormat.NONE) {
        return [
          generateNhDocButton(),
          scanNhDocButton(),
        ];
    ...
    return Column(
      ...
      children: [
        RaisedButton.icon(
          onPressed: pickup.status != Status.ASSIGNED && pickup.status != Status.DRAFT ? null : () => pickupsCubit.startPickup(),
          icon: Icon(Icons.local_shipping),
          label: Text(localizations.translate("pickupDetailsButtonRegisterPickup")),
        ),
        weightButton(),
        ...nhdocButtons(),
        NoteButton(),
