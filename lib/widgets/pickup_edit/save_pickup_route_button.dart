      ...
      onPressed: () {
        if (formKey.currentState!.validate()) {
          ...
          final route = PickupRoute(
              id: routeId,
              name: "PENDING",
              /* Notifies the backend that it should create the order */
              isActive: true,
              plannedDate: DateFormat("yyyy-MM-dd").format(DateTime.now()),
              assignedDriver: driverId,
              assignedOrders: [],
              draftOrders: [],
              car: FirestoreHelper.instance!
                  .doc(FirestorePath.car(transporterId, selectedCar.id)));

          routesCubit.addNewRoute(route).then((_) => navigator.pop(true));
        }
      },
      child: Text(localizations!.translate("pickupRouteSaveTxt")),
    );
  }
}
