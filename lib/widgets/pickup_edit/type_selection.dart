...
class TypeSelection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final origPickup = pickupEditCubit.state.ifState<PickupEditLoaded>(
      withState: (s) => s.oldPickup,
      orElse: (s) =>
          null, // Provide a fallback value if the state is not PickupEditLoaded
    );
        ...
        subStateBuilder: (context, state) {
          ...
          if (dispositions?.isNotEmpty ?? false) {
            ...
            if (finalDisposition == null &&
                origPickup?.finalDispositionId != null) {
              finalDisposition = dispositions.firstWhere(
                  (element) => element.id == origPickup.finalDispositionId);
            }
            return Text(finalDisposition.type);
          } else {
            return Text(localizations!
                .translate("typeSelectionDispositionsNotAvailable"));
          }
        },
