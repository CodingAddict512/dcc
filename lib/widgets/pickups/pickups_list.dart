...
//         new HashMap<String, List<Pickup>>();
...
//           return PickupGroup(customerId2customer[customerId]!,
...
class PickupsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (pickups.isEmpty) {
      print("No pickups found, displaying 'No Pickups' message.");
      return Center(
        child: Text(
          DccLocalizations.of(context)!.translate("pickupsListNoPickups"),
        ),
    ...
    for (Pickup pickup in pickups) {
      String? customerId = pickup.originCustomer?.id;
      if (customerId == null) continue; // Skip if customerId is null
