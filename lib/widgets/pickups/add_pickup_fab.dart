...
class AddPickupFab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool showL10nErrorMessage(String message) {
      ...
      return false; // Ensure the function always returns a bool
    }
    ...
    Pickup placeholderPickup() {
      ...
      //     metricTypeCubit.state.ifState<MetricTypeStateLoaded>(
      ...
      MetricType metricType =
          metricTypeCubit.state.ifState<MetricTypeStateLoaded>(
        withState: (s) =>
            s.metricTypes.first, // Adjust according to new structure
        orElse: (s) => null,
      );
      FinalDisposition finalDisposition =
          finalDispositionCubit.state.ifState<FinalDispositionStateLoaded>(
      ...
      Customer defaultReceiver =
          customersCubit.state.ifState<CustomerStubsLoaded>(
      ...
      return Pickup(
        ...
        collectedTime: Timestamp(200, 200),
        description: "",
        deviceId: "",
        note: "",
        id: id,
        orderId: "DRAFT",
        /* Ensures that the backend leaves it alone for now */
        route: route,
        amount: 1,
        metric: metricType.metric ?? 0,
        metricTypeId: metricType.id ?? 0,
        finalDisposition: finalDisposition.type,
        finalDispositionId: finalDisposition.id,
        actualAmount: 1,
        actualMetric: metricType.metric ?? 0,
        actualMetricTypeId: metricType.id ?? 0,
        ...
        SpeedDialChild(
          ...
          onTap: () async {
            if (!routesCubit.hasSelectedRoute()) {
              showL10nErrorMessage(
                  "pickupFabCreatePickupErrorSelectedRouteNotAvailable");
              return;
            }
        ...
        SpeedDialChild(
          ...
          onTap: () async {
            bool allOk = routesCubit.state.ifState<RoutesLoaded>(
                  withState: (s) => true,
                  orElse: (s) => showL10nErrorMessage(
                      "pickupFabCompleteRouteErrorLoading"),
                ) &&
                pickupCubit.state.ifState<PickupsLoaded>(
                  withState: (s) {
                    if (s.pickups
                        .any((element) => element.status.isInProgress())) {
                      return showL10nErrorMessage(
                          "pickupFabCompleteRouteErrorPickupInProgress");
                    }
                    if (s.pickups
                        .any((element) => element.status != Status.COLLECTED)) {
                      return showL10nErrorMessage(
                          "pickupFabCompleteRouteErrorAllMustBeCollected");
                    }
                    return true;
                  },
                  orElse: (s) => showL10nErrorMessage(
