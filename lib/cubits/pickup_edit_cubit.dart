...
class PickupEditCubit extends Cubit<PickupEditState> {
  ...
  void _setupPickupListener() {
    _listener?.cancel();
    _listener = pickupsCubit.listen((newPickupState) {
      final currentState = state is PickupEditLoaded ? state as PickupEditLoaded : null;
      final currentPickup = currentState?.oldPickup;
      if (currentPickup == null) {
        return;
      }
      newPickupState.ifState<PickupsLoaded>(withState: (s) {
  ...
  Future<void> save() async {
    state.ifState<PickupEditLoaded>(withState: (state) {
  ...
  void registerWeight(
    ...
    state.ifState<PickupEditLoaded>(withState: (state) async {
