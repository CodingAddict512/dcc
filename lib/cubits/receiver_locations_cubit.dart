...
class ReceiverLocationsCubit extends Cubit<ReceiverLocationsState> {
  ...
  ReceiverLocationsCubit({@required this.repository, @required this.pickupEditCubit}) : super(ReceiverLocationsInitial()) {
  ...
  factory ReceiverLocationsCubit.fromContext(BuildContext context) => ReceiverLocationsCubit(
  ...
  void _setupStream() async {
    ...
    if (pickupEditState is PickupEditLoaded && pickupEditState.receiverCustomer != null) {
      listener = repository.getLocations(pickupEditState.receiverCustomer.id).listen((locations) {
        state.ifState<ReceiverLocationsLoaded>(withState: (state) async {
          final loaded = state.copyWith(locations: locations);
          emit(loaded);
        });
        state.ifState<ReceiverLocationsLoading>(withState: (state) async {
