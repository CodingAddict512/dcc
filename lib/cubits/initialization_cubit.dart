...
class InitializationCubit extends Cubit<InitializationState> {
  ...
  factory InitializationCubit.fromContext(BuildContext context) => InitializationCubit(
  ...
  Future<void> _startStateMachine() async {
    ...
    this.selfSubscription = this.listen(this._progressStateUpdate);
  }

  Future<void> _progressStateUpdate(InitializationState newState) async {
    if (newState is InitializationProgressState || newState is InitializationFinalState || newState is InitializationPendingExternalChangeState) {
      return;
    }
    if (externalStream != null) {
      emit(_error("Internal error: We are waiting for an external state change while trying to progress!?"));
      return;
    }
    Function handler = _findNextHandler(state);
    if (handler == null) {
      emit(_error("Internal error: No handler defined for state: " + state.toString()));
    } else {
      try {
        await handler();
        ...
        emit(_error("Internal error: Handler for state threw an exception: " + e.toString()));
      }
    }
  }
  ...
  void setupAwaitLoginListener() {
    assert(externalStream == null);
    externalStream = userCubit.listen((state) {
  ...
  Future<void> _initializeUser() async {
    ...
    if (userCubit.state is UserLoggedIn) {
      ...
      FirestoreConfiguration firestoreConfiguration = await sharedPreferencesRepository.getFirestoreConfiguration();
      String username = await secureStorageRepository.getUsername();
      String password = await secureStorageRepository.getPassword();

      if (firestoreConfiguration == null || username == null || password == null) {
        developer.log("No credentials - awaiting user", name: "initialization_cubit");
        ...
        developer.log("Existing credentials found, trying them ...", name: "initialization_cubit");
        await userCubit.login(firestoreConfiguration, username, password);
        if (userCubit.state is UserLoggedIn) {
          developer.log("Login successful", name: "initialization_cubit");
          emit(InitializationUserInitialized());
        } else {
          developer.log("Login failed: " + userCubit.state.toString(), name: "initialization_cubit");
          assert(userCubit.state is UserError);
          setupAwaitLoginListener();
          emit(InitializationUserNeedsReset(userError: userCubit.state as UserError));
        }
      }
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      settingsCubit.state.ifState<SettingsLoaded>(orElse: (s) async => await settingsCubit.loadSettings());
    } catch (e) {
