...
class OriginLocationsCubit extends Cubit<OriginLocationsState> {
  ...
  factory OriginLocationsCubit.fromContext(BuildContext context) =>
      OriginLocationsCubit(
  ...
  void _setupStream() async {
    ...
    if (pickupEditState is PickupEditLoaded &&
        pickupEditState.originCustomer != null) {
      listener = repository
          .getLocations(pickupEditState.originCustomer.id)
          .listen((locations) {
        state.ifState<OriginLocationsLoaded>(
        ...
        state.ifState<OriginLocationsLoading>(
          withState: (state) async {
