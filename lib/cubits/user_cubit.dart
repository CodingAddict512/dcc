...
abstract class IUserCubit extends Cubit<UserState> {
  ...
  Future<void> login(FirestoreConfiguration firestoreConfiguration, String username, String password) async {
    emit(UserLoading());
    final result = await signInWithEmailAndPassword(firestoreConfiguration, username, password);
    emit(result);
  }

  Future<UserState> devLoginWithEmailAndPassword(FirestoreConfiguration firestoreConfiguration, String email, String password) async {
    UserState state = await signInWithEmailAndPassword(firestoreConfiguration, email, password);
    emit(state);
    return state;
  }
  ...
  @protected
  Future<UserState> signInWithEmailAndPassword(FirestoreConfiguration firestoreConfiguration, String username, String password);
...
class UserCubit extends IUserCubit {
  ...
  Future<UserState> devLoginWithEmailAndPassword(FirestoreConfiguration firestoreConfiguration, String email, String password) async {
    UserState state = await super.devLoginWithEmailAndPassword(firestoreConfiguration, email, password);
    if (!(state is UserError)) {
      await sharedPreferencesRepository.setFirestoreConfiguration(
  ...
  Future<UserState> signInWithEmailAndPassword(FirestoreConfiguration firestoreConfiguration, String email, String password) async {
    try {
      User user = await userRepository.signInWithEmailAndPassword(firestoreConfiguration, email, password);
      ...
      return UserError.fromException(e);
    }
  }

  Future<UserState> performOTPLogin(String email, String otp, String verifyUrl) async {
    try {
      Tuple2<FirestoreConfiguration, String> result = await userRepository.verifyOtp(otp, verifyUrl);
