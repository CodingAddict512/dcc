...
class PrimaryLocationsCubit extends Cubit<PrimaryLocationsState> {
  ...
  PrimaryLocationsCubit({required this.repository})
      : super(PrimaryLocationsInitial()) {
    _setupStream();
  }

  factory PrimaryLocationsCubit.fromContext(BuildContext context) =>
      PrimaryLocationsCubit(
  ...
  void _setupStream() async {
    ...
    listener = repository.getCustomerStubs().listen((customerStubs) {
      state.ifState<PrimaryLocationsLoaded>(
        withState: (s) {
      ...
      state.ifState<PrimaryLocationsLoading>(
        withState: (s) {
