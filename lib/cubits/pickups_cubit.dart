...
class PickupsCubit extends Cubit<PickupsState> {
  ...
  void updateStream() async {
    ...
    if (userCubitState is UserLoggedIn && routesCubitState is RoutesLoaded) {
      ...
      Stream<List<PickupWithReferences>> pickupsStream = pickupRepository.getPickupsWithRefs(
      ...
      _listener = stream.listen((pickups) {
        ...
        List<DocumentReference> draftOrders = routesCubitState.route?.draftOrders ?? [];
        final absentIds = draftOrders.where((element) => !allActiveIds.contains(element.id)).map((e) => e.id).toList();
        ...
        state.ifState<PickupsLoading>(withState: (state) {
  ...
  void selectPickup(Pickup pickup) {
    state.ifState<PickupsLoaded>(withState: (state) async {
  ...
  void startPickup() async {
    state.ifState<PickupsLoaded>(withState: (state) async {
      ...
      String newOrderId = state.pickup.status == Status.DRAFT ? "PENDING" : state.pickup.orderId;
      final newPickup = state.pickup.copyWith(orderId: newOrderId, status: Status.STARTED);
  ...
  void finishPickup() async {
    state.ifState<PickupsLoaded>(withState: (state) async {
      ...
      final newPickup = pickup.copyWith(
        ...
        actualFinalDisposition: pickup.actualFinalDisposition ?? pickup.finalDisposition,
        actualFinalDispositionId: pickup.actualFinalDispositionId ?? pickup.finalDispositionId,
        deviceId: deviceId,
      );
      await _setNewPickup(newPickup);
    });
