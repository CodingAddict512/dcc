...
class UserRepository implements IUserRepository {
  ...
  Future<User> signInWithEmailAndPassword(FirestoreConfiguration firestoreConfiguration, String email, String password) async {
    ...
    developer.log("Performing sign-in via FirebaseAuthHelper ...", name: "user_repository");
    fb.User user = await FirebaseAuthHelper.signInWithEmailAndPassword(email, password);
    developer.log("Sign in complete - Fetching the user ID token ...", name: "user_repository");
  ...
  Future<Tuple2<FirestoreConfiguration, String>> verifyOtp(String otp, String verifyUrl) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception("Unexpected status code in otp response: ${resp.statusCode}.");
    }
