...
class FirestoreService {
  ...
  static Stream<List<PickupRoute>> routesStream({
    ...
    return FirestoreHelper.collectionStream(
      sort: (PickupRoute a, PickupRoute b) => a.name.compareTo(b.name),
  ...
  static Stream<List<PickupWithReferences>> pickupsWithPrefsStream({
    ...
    return FirestoreHelper.collectionStream(
      sort: (PickupWithReferences a, PickupWithReferences b) =>
          a.amount.compareTo(b.amount),
      path: FirestorePath.pickups(),
      queryBuilder: (query) => query
          .where("driverId", isEqualTo: driverId)
          .where("route", isEqualTo: routeRef),
      builder: (data, documentId) =>
          PickupWithReferences.fromMap(data, documentId),
    );
  }
  ...
  static Future<void> deletePickup({
    required Pickup pickup,
      ...
      await FirestoreHelper.instance!
          .doc(FirestorePath.pickup(pickup.id))
          .delete();
  ...
  static Future<PickupWithReferences> setPickup({
    ...
    final originLocation =
        pickup.originCustomer != null && pickup.originLocation != null
    ...
    final receiverLocation =
        pickup.receiverCustomer != null && pickup.receiverLocation != null
            ? FirestorePath.location(
                pickup.receiverCustomer!.id,
            ...
            : null;
    final route =
        pickup.route != null ? FirestorePath.route(pickup.route!.id) : null;
    ...
    Future<DocumentReference> originLocationRef() async {
      ...
      // return null;
      throw Exception("originLocation cannot be null");
    }
    ...
    final pickupWithRefs = PickupWithReferences(
      ...
      originCustomer: originCustomer != null
          ? FirestoreHelper.instance!.doc(originCustomer)
          : null!,
      originLocation: await originLocationRef(),
      receiverCustomer: receiverCustomer != null
          ? FirestoreHelper.instance!.doc(receiverCustomer)
          : null!,
  ...
  //   return FirestoreHelper.collectionStream(
  //     sort: (Car a, Car b) => a.id.compareTo(b.id),
  //     path: FirestorePath.customers(),
  //     builder: (data, documentId) => Customer.fromMap(data, documentId),
  //   ).asyncMap((futures) => Future.wait(futures));
  // }
  static Stream<List<Customer>> customersStream() {
    return FirestoreHelper.collectionStream<Map<String, dynamic>>(
      sort: (Map<String, dynamic> a, Map<String, dynamic> b) {
        return a['id'].compareTo(b['id']);
      },
      path: FirestorePath.customers(),
      queryBuilder: (query) =>
          query.where("isActive", isEqualTo: true), // Example query
      builder: (data, documentId) {
        // Returning a map of data and documentId
        return {'data': data, 'id': documentId};
      },
    ).asyncMap((documents) async {
      // Convert each Map to Customer asynchronously
      final List<Customer> customers = await Future.wait(
        documents.map((doc) async {
          final data = doc['data'] as Map<String, dynamic>;
          final id = doc['id'] as String;
          return await Customer.fromMap(data, id);
        }),
      );

      // Sort the customers by ID after mapping
      customers.sort((Customer a, Customer b) => a.id.compareTo(b.id));
  ...
  static Stream<List<Location>> locationsStream({
    required String customerId,
      ...
      FirestoreHelper.collectionStream(
        sort: (Location a, Location b) => a.id.compareTo(b.id),
  ...
  static Stream<List<CustomerStub>> customerStubsStream() {
    return FirestoreHelper.documentStream(
      ...
      builder: (data, documentID) =>
          data != null ? data["default-locations"] : [],
    ).asyncMap((dls) {
      List<Future<CustomerStub>> dlsFutures = dls
          .map((dl) => dl as Map<String, dynamic>)
          .map<Future<CustomerStub>>((dl) => CustomerStub.fromMap(dl))
          .toList();
  ...
  static Stream<List<FinalDisposition>> finalDispositionsStream() {
    return FirestoreHelper.documentStream(
      ...
      builder: (data, documentID) =>
          data != null ? data["final-dispositions"] as List<dynamic> : [],
    ).asyncMap((dls) {
      final v = dls
          .map((e) => e as Map<String, dynamic>)
          .map((e) => FinalDisposition.fromMap(e))
          .toList();
      return v;
    });
  }
  ...
  static Stream<List<MetricType>> metricTypesStream() {
    return FirestoreHelper.documentStream(
      ...
      builder: (data, documentID) =>
          data != null ? data["metric-types"] as List<dynamic> : [],
    ).asyncMap((dls) {
      final v = dls
          .map((e) => e as Map<String, dynamic>)
          .map((e) => MetricType.fromMap(e))
          .toList();
      return v;
    });
  }
