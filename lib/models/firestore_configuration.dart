...
class FirestoreConfiguration {
  ...
  // Factory method to create a FirestoreConfiguration from a string value
  factory FirestoreConfiguration.fromPreferenceValue(String value) {
    List<String> values = value.split("\x1f");
    if (values.length != 5 || values[0] != "v1") {
      throw Exception("Invalid firestore configuration");
    }
    return FirestoreConfiguration(
      apiKey: values[1],
      googleAppId: values[2],
  ...
  // Method to convert FirestoreConfiguration to a string value
  String toPreferenceValue() {
    return "v1" +
        "\x1f" +
        apiKey +
