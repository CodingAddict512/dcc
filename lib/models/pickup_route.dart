import 'package:cloud_firestore/cloud_firestore.dart';

class PickupRoute {
  final String id;
  final String name;
  final String assignedDriver;
  final bool isActive;
  final String plannedDate;
  final List<DocumentReference> assignedOrders;
  final List<DocumentReference> draftOrders;
  final DocumentReference car;

  PickupRoute({
    required this.id,
    required this.name,
    required this.assignedDriver,
    required this.isActive,
    required this.plannedDate,
    List<DocumentReference>? assignedOrders,
    List<DocumentReference>? draftOrders,
    required this.car,
  })  : assignedOrders = assignedOrders ?? [],
        draftOrders = draftOrders ?? [];

  factory PickupRoute.fromMap(Map<String, dynamic> data, String documentId) {
    final assignedOrders = _asListOfReferences(data["assignedOrders"]);
    final assignedOrderIds = assignedOrders.map((e) => e.id).toSet();
    final draftOrders = _asListOfReferences(data["draftOrders"])
        .where((element) => !assignedOrderIds.contains(element.id))
        .toList();

    return PickupRoute(
      id: documentId,
      name: data["name"].toString(),
      assignedDriver: data["assignedDriver"],
      isActive: data["isActive"],
      plannedDate: data["plannedDate"],
      assignedOrders: assignedOrders,
      draftOrders: draftOrders,
      car: data["car"],
    );
  }

  static Future<PickupRoute> fromReference(DocumentReference reference) async {
    final snapshot = await reference.get();
    final data = snapshot.data() as Map<String, dynamic>?;

    if (data == null) {
      throw Exception("Document does not exist or data is null");
    }

    return PickupRoute.fromMap(data, reference.id);
  }

  List<DocumentReference> get allOrders =>
      combinedReferences(assignedOrders, draftOrders);

  // Convert PickupRoute to a Map for Firestore
  Map<String, dynamic> toMap() {
    final assignedOrderIds = assignedOrders.map((e) => e.id).toSet();
    final draftOrdersList = draftOrders
        .where((element) => !assignedOrderIds.contains(element.id))
        .toList();

    return {
      "id": id, // Change from name to id for correct mapping
      "isActive": isActive,
      "plannedDate": plannedDate,
      "assignedDriver": assignedDriver,
      "assignedOrders": assignedOrders.map((docRef) => docRef.id).toList(),
      "car": car.id,
      "draftOrders": draftOrdersList.map((docRef) => docRef.id).toList(),
    };
  }

  // Convert PickupRoute to JSON format
  Map<String, dynamic> toJson() {
    return {
      "id": id,
      "name": name,
      "assignedDriver": assignedDriver,
      "isActive": isActive,
      "plannedDate": plannedDate,
      "assignedOrders": assignedOrders.map((docRef) => docRef.id).toList(),
      "car": car.id,
      "draftOrders": draftOrders.map((docRef) => docRef.id).toList(),
    };
  }

  bool isCreation() => name == "PENDING" || name == "ERROR";

  static List<DocumentReference> _asListOfReferences(dynamic data) {
    return data
            ?.map<DocumentReference>((d) => d as DocumentReference)
            ?.toList() ??
        [];
  }

  static List<DocumentReference> combinedReferences(
      List<DocumentReference> refs, List<DocumentReference> additionalRefs) {
    final knownIds = refs.map((e) => e.id).toSet();
    final combinedList = List<DocumentReference>.from(refs);
    combinedList.addAll(
        additionalRefs.where((element) => !knownIds.contains(element.id)));
    return combinedList;
  }

  // Add copyWith method
  PickupRoute copyWith({
    String? id,
    String? name,
    String? assignedDriver,
    bool? isActive,
    String? plannedDate,
    List<DocumentReference>? assignedOrders,
    List<DocumentReference>? draftOrders,
    DocumentReference? car,
  }) {
    return PickupRoute(
      id: id ?? this.id,
      name: name ?? this.name,
      assignedDriver: assignedDriver ?? this.assignedDriver,
      isActive: isActive ?? this.isActive,
      plannedDate: plannedDate ?? this.plannedDate,
      assignedOrders: assignedOrders ?? this.assignedOrders,
      draftOrders: draftOrders ?? this.draftOrders,
      car: car ?? this.car,
    );
  }
}
