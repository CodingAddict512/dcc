...
class ExternalNhDocPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      return context.watch<PickupsCubit>().state.ifState<PickupsLoaded>(
        withState: (state) {
          return ElevatedButton.icon(
            onPressed: (state.pickup.status == Status.STARTED || state.pickup.status == Status.REJECTED)
                ? () async {
            ...
            label: Text(DccLocalizations.of(context).translate("externalNhDocPageRenewNhDoc")),
          );
        },
        orElse: (state) {
          return Container();
    ...
    Widget displayNhDoc() {
      return BlocSubStateBuilder<ExternalNhDocCubit, ExternalNhDocState, ExternalNhDocLoadedInMemory>(
        ...
        fallbackBuilder: (context, state) {
          ...
          if (state is ExternalNhDocUploadError) {
            pickupsCubit.state.ifState<PickupsLoaded>(withState: (state) {
              externalNhDocCubit.downloadNhDocImage(state.pickup);
            });
