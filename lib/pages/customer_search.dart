...
class CustomerSearchState extends State<CustomerSearch> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<CustomerStub>> search(String query) async {
      ...
      return primaryLocationsCubit.state.ifState<PrimaryLocationsLoaded>(
        withState: (state) async {
          ...
          List<PrimaryLocationDistance> sorted = primaryLocationsCubit.sortByDistance(_currLocation, filtered);
          ...
          return sorted.map((dld) => dld.primaryLocation)
                      .toList();
        },
    ...
    Future<GeoPoint> getAndUpdateCurrLocation() async {
      try {
        GeoPoint newCurrLocation = await geoLocationCubit.getCurrentGeoPosition();
        if (mounted) {
          setState(() {
    ...
    Widget searchField() {
      return TextFormField(
        ...
        decoration: InputDecoration(
          hintText: DccLocalizations.of(context).translate("customerSearchSearchFieldHint"),
    ...
    Widget clearSearchFieldButton() {
      return IconButton(
        ...
        tooltip: DccLocalizations.of(context).translate("customerSearchClearTooltip"),
