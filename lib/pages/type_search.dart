...
class TypeSearchState extends State<TypeSearch> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<FinalDisposition>> search(String query) async {
      ...
      return finalDispositionEditCubit.state.ifState<FinalDispositionStateLoaded>(
        withState: (state) async {
          Iterable<FinalDisposition> filtered = state.dispositions.where((l) {
            return l.isActive && l.type.toLowerCase().contains(query.toLowerCase());
          });
    ...
    Widget clearSearchFieldButton() {
      return IconButton(
        ...
        tooltip: DccLocalizations.of(context).translate("typeSearchClearTooltip"),
