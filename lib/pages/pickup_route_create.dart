...
class PickupRouteCreateState extends State {
  ...
  Widget build(BuildContext context) {
    ...
    if (userCubit.state is UserLoggedIn && !(carsCubit.state is CarsLoaded || carsCubit.state is CarsLoading)) {
      userCubit.state.ifState<UserLoggedIn>(withState: (s) => carsCubit.loadCars(s.transporterId));
    }
    ...
    Widget _carSelectionTitle() {
      return BlocSubStateBuilder<CarsCubit, CarsState, CarsLoaded>(
        subStateBuilder: (context, state) {
          ...
          if (allCars != null && allCars.isNotEmpty) {
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              child: Row(
                children: [
                  SavePickupRouteButton(formKey: _formKey, routeId: routeId,)
                ],
              ),
            )
          ],
